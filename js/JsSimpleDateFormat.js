...
parse: function(s,isNN) {
	...
	else while(c >= '0' && c <= '9') { //If in the end of pattern, include all the rest numeric character (this._count is ignored)
		sVal += c;
		if (++i < s.length) c = s.charAt(i); else break;
	}
	if (i == 0) return -1;
...
parse: function(s,oPos) {
	...
	while (j < arPtn.length) {
		...
		if (oPtn instanceof JsSimpleDateFormat._Ltr) { 
			var sFN = oPtn.name;
			if (oFields[sFN]) { //There has been the field whose the same name
