	...
	Datepicker.prototype = {
		...
		_process_options: function(opts){
			...
			if (!$.isArray(o.datesDisabled)) {
				var datesDisabled = [];
				datesDisabled.push(DPGlobal.parseDate(o.datesDisabled, o.format, o.language));
				o.datesDisabled = datesDisabled;
			}
			o.datesDisabled = $.map(o.datesDisabled,function(d){
				return DPGlobal.parseDate(d, o.format, o.language);
			});
		...
		getClassNames: function(date, time){
			var cls = [],
		...
		fill: function(){
			...
			if (this.o.weekPicker) {
				...
				$dayRows.mouseover(function(e) { //using mouseover, not mouseenter is to adapt disabled dates
					...
					$tr.css('background-color', $tr.find('td:hover').css('background-color')); //always sets bgcolor because disabled and enabled date's color are different
					...
					case 'th':
						switch (target[0].className){
							...
							case 'next':
								...
								break;
							/*case 'time':
								this._timeChange(e.target, false);*/
						}
						break;
					case 'span':
						if (target.hasClass('today') || target.hasClass('now')) {
							var d = new Date();
							var date = UTCDate(d.getFullYear(), d.getMonth(), d.getDate(), 0, 0, 0);
							this.showMode(-2);
							...
							var which = opt === 'linked' ? null : 'view';
						...
						break;
					case 'td':
						if (target.hasClass('day') && !target.hasClass('disabled')) this._setDate(this._getSelectedDate(target));
						break;
				}
			}
		...
		_getSelectedDate: function($target) {
	...
	var DPGlobal = {
		...
		_parseDateDiff: function(date){
			if (/^[\-+]\d+[dmMwy]([\s,]+[\-+]\d+[dmwy])*$/.test(date)){
				var part_re = /([\-+]\d+)([dmMwy])/, parts = date.match(/([\-+]\d+)([dmMwy])/g);
				date = new Date();
				for (var i=0; i < parts.length; i++){
					var part = part_re.exec(parts[i]);
					var dir = parseInt(part[1]);
					switch (part[2]){
						case 'd':
		...
		parseDate: function(date, format, language){
			...
			if (dt) return dt;
			var part, i, parts = date && date.match(this.nonpunctuation) || [];
			date = new Date();
			var parsed = {},
